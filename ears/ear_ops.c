 ...
 */
snd_pcm_t* init_ear(){
    ear_params* param = get_default_ear_params();

    snd_pcm_t* playback_handle = init_ear_with_param(param);
    release_ear_params(param);
...
void recode_to_buffer(snd_pcm_t *playback_handle, void *buffer, int size){
    int err;
    if((err = snd_pcm_readi(playback_handle,buffer,size)) != size){
